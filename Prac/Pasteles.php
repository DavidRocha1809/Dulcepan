<?php
    ...
    //Ejecuta sentencia cuando la disponibilidad de un pan == 1 en la BD
    $sql = $con->prepare("SELECT idPasteles, Nombre, Precio FROM pasteles WHERE disponibilidad=1");
    $sql->execute();
...
<head>
    ...
    <link rel="stylesheet" href="StylePasteles.css">
...
<body>
    ...
    <div class="titulo">
        <h2>¡Bienvenido a la selección de PASTELES!</h2>
    </div>
    ...
    <div class="contenedor">
        <?php foreach ($resultado as $row) { ?>
            <div class="tarjeta">
                <?php
                    //busca el id de cada pan para abrir la carpeta correspondiente a su imagen
                    $id = $row['idPasteles'];
                    $imagen = "Productos_pasteles/" . $id . "/pastel.jpg";

                    //Si el valor id no tiene una carpeta con img, agrega una img "dañada"
                    if(!file_exists($imagen)){
                        $imagen = "Productos_pasteles/no-photo.png";
                    }
                ...
                <div class="tarjeta-body">
                    ...
                    <form method="post" action="agregar_carrito.php">
                        <input type="hidden" name="id" value="<?php echo $row['idPasteles']; ?>">
                        <input type="hidden" name="nombre" value="<?php echo $row['Nombre']; ?>">
                        <input type="hidden" name="precio" value="<?php echo $row['Precio']; ?>">
                        <input type="hidden" name="cantidad" value="1" class="cantidad">
                        <input type="hidden" name="categoria" value="pastel">
